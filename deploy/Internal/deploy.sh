project_dir=$(pwd)
current_dir=$project_dir/deploy/Internal

echo "Using version: $ECR_TAG"

ecr=001043692857.dkr.ecr.eu-central-1.amazonaws.com
dockerImage=$ecr/pm-platform:int-$ECR_TAG
...
#Update
aws ecs update-service --cluster PM-Internal-Cluster --service PM-Platform-Service --task-definition $taskDefinitionArn --force-new-deployment > /dev/null && \
